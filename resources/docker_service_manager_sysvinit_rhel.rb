...
resource_name :docker_service_sysvinit_rhel

provides :docker_service, platform: 'amazon'
provides :docker_service, platform: 'suse'
provides :docker_service, platform: %w(redhat centos scientific oracle) do |node| # ~FC005
  node['platform_version'].to_f <= 7.0
end

provides :docker_service_sysvinit, platform: 'amazon'
provides :docker_service_sysvinit, platform: 'suse'
provides :docker_service_sysvinit, platform: %w(redhat centos scientific oracle) do |node| # ~FC005
  node['platform_version'].to_f <= 7.0
end
