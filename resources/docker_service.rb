...
action_class.class_eval do
  ...
  def svc_manager(&block)
    ...
    when 'auto'
      svc = docker_service(new_resource.name, &block)
    when 'execute'
      svc = docker_service_execute(new_resource.name, &block)
    when 'sysvinit'
      svc = docker_service_sysvinit(new_resource.name, &block)
    when 'upstart'
      svc = docker_service_upstart(new_resource.name, &block)
    when 'systemd'
      svc = docker_service_systemd(new_resource.name, &block)
    end
