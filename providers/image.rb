...
def load_current_resource
  ...
  di = shell_out("docker images -a", :timeout => new_resource.timeout || 60)
...
action :pull do
  unless installed?
    ...
    shell_out("docker pull #{new_resource.image_name} #{pull_args}", :timeout => new_resource.timeout || 60)
...
action :build do
  unless installed?
    ...
    shell_out("docker build -t #{full_image_name} - < #{new_resource.dockerfile}", :timeout => new_resource.timeout || 60)
    new_resource.updated_by_last_action(true)
  end
end
...
def remove
  shell_out("docker rmi #{new_resource.image_name}", :timeout => new_resource.timeout || 60)
end
